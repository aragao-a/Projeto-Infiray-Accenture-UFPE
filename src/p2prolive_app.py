...
@st.cache_resource
def load_detector():
    """Carrega o detector de anomalia térmica uma única vez."""
    try:
        detector = ThermalDetector(model_path=YOLO_MODEL_PATH)
        return detector
    except Exception as e:
        st.error(f"Erro ao carregar o ThermalDetector: {e}")
        st.stop()
...
while True:    # main aquisition loop
    ...
    if temp_max > temp_min:
        # Cria a imagem 0-255 (uint8) para a função anomaly_score
        temp_norm_u8 = ((temp - temp_min) / (temp_max - temp_min) * 255).astype(np.uint8)
    else:
        temp_norm_u8 = np.zeros_like(temp, dtype=np.uint8)

    rgb_for_yolo = detector.to_rgb(temp_norm_u8)

    results = detector.detect_objects(rgb_for_yolo, thermal_data=temp_norm_u8)
    
    thermal_anomalies = results['thermal_anomalies']
    ...
    vis = np.array(im.convert('RGB')) # 'vis' é a imagem colorida de fundo
    vis = cv2.cvtColor(vis, cv2.COLOR_RGB2BGR) # Convertendo de RGB para BGR (padrão OpenCV)
    ...
    for detection in all_detections:
        ...
        anomaly_data = detection['thermal_anomaly']
        alert = anomaly_data.get('has_anomaly', False)
        score = anomaly_data.get('score', 0.0)
        
        # VERMELHO se Alerta, VERDE se Detecção Normal
        color = (0, 0, 255) if alert else (0, 255, 0) 
        
        cv2.rectangle(vis, (x1, y1), (x2, y2), color, 2)
        
        if alert:
            p95 = anomaly_data['p95']
            mean_roi = anomaly_data['mean_roi']
            mean_bg = anomaly_data['mean_bg']
            label = f"{cls_name} c{conf:.2f} | P95:{p95:.0f} BG:{mean_bg:.0f} S:{score:.1f} 🚨"
        else:
            label = f"{cls_name} c{conf:.2f}"
